...
export const createMaintenanceController = async (req: Request, res: Response) => {
    try {
        ...
        if (!createMaintenance) {
            res.status(400).json({ message: "Maintenance record creation failed" });
            return;
        }
        res.status(201).json({ message: "Maintenance record created successfully", maintenance: createMaintenance });
    } catch (error) {
...
export const updateMaintenanceController = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).json({ message: "Maintenance record updated successfully", maintenance: updatedMaintenance });
    } catch (error) {
...
export const getCarMaintenanceHistoryController = async (req: Request, res: Response) => {
    try {
        ...
        if (!maintenanceHistory || maintenanceHistory.length === 0) {
            res.status(404).json({ message: "No maintenance records found for this car" });
            return;
        }
